...
| 03.01 | Code refactor to jakarta and Security 6 | Success | [03.01 In-progress] Performed repository-wide scan for javax.* usages (persistence, validation, servlet). [03.01 Success] All javax.persistence.* replaced with jakarta.persistence.* in model entities; no javax.validation.* or javax.servlet.* references found in codebase. Files checked/changed: src/main/java/com/coding/exercise/bankapp/model/*.java (Account.java, Address.java, BankInfo.java, Contact.java, Customer.java, CustomerAccountXRef.java, Transaction.java) — already on jakarta.*; SecurityConfig validated for Spring Boot 3 APIs. |
| 03.02 | Update Spring Security to Spring Security 6 style | Success | [03.02 In-progress] Migrated security configuration to Spring Security 6 style with SecurityFilterChain and requestMatchers. [03.02 Success] Permitted OpenAPI (/v3/api-docs/**), Swagger UI (/swagger-ui/**, /swagger-ui.html), H2 console (/h2-console/**), and Actuator (/actuator/**); enabled HTTP Basic; disabled CSRF and frame options for H2. Files changed: src/main/java/com/coding/exercise/bankapp/config/SecurityConfig.java |
