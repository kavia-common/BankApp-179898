...
- `SecurityConfig` – Spring Security 6 configuration using `SecurityFilterChain`:
  - Permits unauthenticated access to:
    ...
    - `/swagger-ui.html`
    - `/h2-console/**`
  - Business endpoints may be open in dev profile; re-enable Basic auth as needed.
...
- `application.yml`:
  - Sets `server.port: 3001`
  - Sets `server.servlet.context-path: /bank-api`
  - Configures an in-memory H2 console (enabled).
  - Defines default Spring Security user credentials (may be unused in dev).
...
Run from the module (repository root for this container):
...
- When adding new endpoints, update controllers, services, DTOs, and OpenAPI annotations where appropriate.
